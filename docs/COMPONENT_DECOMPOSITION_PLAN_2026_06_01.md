...
| 6 | **`FrameDemodulator`** | samples→soft-bits demod-prep — owns the CFO pre-correction (`applyCFOPreCorrection` + `pre_correction_cfo_`); grows to the waveform.process orchestration | 🔶 SEEDED (CFO pre-correction carved, §7 C-FD-1) |
| 6b | **`FrameDecoder`** | soft-bits→frame FEC decode — owns the decode primitives (`codec_` + `interleaver_`); grows to own `decodeFrame`/`decodeMCDPSKFrame` (deferred: hottest path + the `DecodeResult` circular-include) | 🔶 FOUNDED (primitives relocated, §7 C-FDec-1) |
