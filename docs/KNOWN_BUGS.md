...
- Status: **ROOT-CAUSED + FIX PROVEN (sim + rig), env-gated `ULTRA_ROBUST_IDLE_PING` default-OFF. STAGE2 responder fix LANDED 2026-07-01 (CHANGELOG); the ORIGINAL responder-starvation blocker is CLOSED. Default-ON now blocked on a NEW, narrower issue: the emit churns the HIGH-SNR handshake (good@20 sim regression — see below).** Rig low-SNR connect proven with the knob ON (MPG@9, MPM@8 pure-default+knob connect + deliver CRC-clean).
