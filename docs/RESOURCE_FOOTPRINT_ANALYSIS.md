...
| FFT state | `src/dsp/fft.cpp` | PocketFFT shape/stride metadata plus its internal plan cache for repeated small transforms | modulator/demodulator/waveforms | Memory is modest; no external FFT runtime is required. OFDM hot paths use pre-sized pointer FFT calls instead of vector-overload resize. |
...
Static binary size on macOS ARM64 with SDL:
...
| --- | --- | --- | --- |
| Pi Zero 2W | 1 GHz quad Cortex-A53, 512 MB, Linux audio scheduling weaker than Pi 5 | Headless build, no GUI, ring-buffer audio queues, profile gate on sync/control decode, keep a fast PocketFFT/NEON path | FEASIBLE WITH WORK |
...
| BeagleBone Black/Green | 1 GHz single Cortex-A8, 512 MB, old CPU, practical audio I/O exists | Must cut sync/control spikes, remove per-callback allocations, keep fixed-point/NEON expectations realistic | FEASIBLE WITH MAJOR WORK |
