...
| `ULTRA_WARM_TURNAROUND` | make the connected-OFDM pre-TX echo-clear (`clearRxBuffer(for_tx_echo=true)`, fired before every ACK turnaround) a decoder NO-OP, preserving warm-sync state + ring timeline across the half-duplex turnaround instead of `reset()`-wiping them (which forced a COLD 2.5s re-acquire of every next burst — the ~27% Good/Moderate turnaround tax). The audio side (`setRxMuted`+`stopCapture`+`AudioEngine::clearRxBuffer`) already prevents echo, so the decoder full-reset was harmful overkill; the no-op makes the rig behave like OTASim (which skips this path entirely, app.cpp:3040). Lockstep rig A/B (MPG@20): MEDIAN turnaround 2.71s→1.54s (−43%), CRC-clean, both pairs. Real-half-duplex-only correctness change the faithful gate CANNOT see (sim TX skips it) → default-off until multi-channel-proven. The OTASim ~0.6s (warm predict-and-wait) is partly a SIM ARTIFACT (±20ms window vs ±260ms real inter-group jitter) — NOT a target. | **OFF** (2026-06-20, rig-proven −43%, env-gated until wide) | `modem_engine.cpp` `clearRxBuffer`; call site `app.cpp:3051` | FEAT (prototype) |
