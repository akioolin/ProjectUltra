...
| `ULTRA_ENABLE_QAM16_LADDER` | let the AUTO ladder SELECT 16QAM (Phase 1, 2026-06-12). ON → `selectCoherentOFDM` walks `kCoherentLadderQAM16Exp` with the {QAM16,R1/2} GOOD rung enabled at the Phase-0a measured floor (18 dB, EXPERIMENTAL zero-margin). Default ladder byte-identical when unset. Paired with the code-derived `maxValidatedCoherentRate(mod)` per-mod rate cap (QAM16→R1/2) so the modulation-blind RateController can't climb into damage-bound 16QAM R2/3+. GUI-verified: gate-off→QPSK unbroken; gate-on→16QAM R1/2 auto-selected+delivered; +adapt→pinned R1/2. | **OFF** | `waveform_selection.hpp` `qam16LadderEnabled()` / `kCoherentLadderQAM16Exp` / `maxValidatedCoherentRate`; cap clamp `connection.cpp` `applyAdaptiveRateFeedback` | FEAT (Phase 1; lateral until Phase 2b makes a denser 16QAM rung clean) |
...
| `ULTRA_HERR_LLR_K` | per-carrier channel-estimate-error (ε²_H) LLR term: `nv=(σ²+k·err_var·|H|²)/(|H|²+σ²)` using the Wiener interpolator's normalized MMSE residual (the pilot-anchored production form of EMP_FLOOR). Down-weights confident-wrong LLRs on poorly-estimated carriers near freq-selective nulls; inert on flat channels (err_var≈0). **GUI-validated 2026-06-12**: 16QAM R2/3 sp8 Good@20 loss 55→45% / +20% goodput 3/3 seeds; QPSK/AWGN no-regress; 16QAM R1/2 +3-8%. **k=1.0 VALIDATED** (k-tune 0.5/1.0/2.0 Good@20: peaks at 1.0; k=2.0 over-inflates, CW-fails spike; k=0.5 under-weights — the Wiener error_var is a calibrated variance, trust it 1:1). | **0 (off)** — ships gated pending wider stacking (relative-null-CSI-for-16QAM, stuck-tail) + full-matrix sweep before default-on / cap-lift past R1/2 | `channel_equalizer_equalize.cpp` `equalize()`; populated `channel_equalizer_pilot.cpp` `updateChannelEstimate`; member `demodulator_impl.hpp per_carrier_h_error_var_` | ADAPT (Phase 2b; the unified per-carrier reliability model) |
