...
- **Operate under the multi-perspective stack at all times.** All technical answers, design decisions, code reviews, and Codex briefs must be written from the *combined and mandatory* standpoint of:
  ...
  **Hard physical constraints (category-error guards — check EVERY design/throughput/timing claim against these before proposing it; violating one means the design is impossible on a real radio, not merely suboptimal):**
  - **Half-duplex** (operator lens above): no TX+RX at once; no pipelining across the ACK gap.
  - **PA duty cycle** (operator lens above): rig must catch air; ~100% duty is a cheat.
  - **Information-theoretic ceiling:** cannot exceed Shannon capacity for the SNR, nor a rung's raw bit-rate (modulation_bits × baud × code_rate × data_carriers). A throughput claim above the rung ceiling or channel capacity is a BUG — state the ceiling before optimizing toward it (e.g. QPSK R2/3 = 2611 bps < leader's 3086, so QPSK R2/3 can NEVER match it regardless of efficiency).
  - **Time-varying channel → stale CSI:** Doppler sets coherence TIME (bounds how long a channel estimate / burst stays valid); delay spread sets coherence BANDWIDTH (bounds pilot spacing, Nyquist for the channel, CP length). You always equalize with a past estimate.
  - **Fading loss is irreducible:** deep nulls destroy frames regardless of estimator quality → ARQ is mandatory and "zero retx on fading" is unphysical. Diversity/FEC/interleaving reduce it; nothing eliminates it.
  - **No shared timebase:** the two rigs' sample clocks are not locked (ppm drift) and carry dial CFO/drift — never assume perfect timing or frequency lock; both are always present and changing.
...
- **Nothing in the codebase is assumed correct — Codex (and Claude) must question and fix anything in scope.** Every brief must instruct Codex: *do not assume any existing code, comment, doc, constant, default, prior "fix", or test expectation is correct from any perspective (PHY, DSP, operator, or first-principles).* This is ~6 months of AI iteration with known stale comments, compensating band-aids, and silent breakage (e.g. Watterson was amplitude-only for months; a comment claimed "no 4-cycles" that didn't hold; the LLR σ² was pinned). Codex must be free — and is expected — to analyze whatever it needs (including code outside the immediate file, the test harness itself, and the simulator's fidelity), follow the evidence to the real root cause, and fix the root cause rather than patch a symptom or defer to "it was probably done for a reason." Treat comments/docs/constants as *claims to verify*, not ground truth; if a comment and the code disagree, find out which is wrong. **This is NOT license to thrash:** every change still passes the proof gate (multi-seed, whole-matrix, no regression, faithful clock) and losers get reverted — fearless analysis, *proven* changes. Run reasoning summaries visible (`-c model_reasoning_summary="detailed"`) so the reasoning can be followed and drift caught early.

- **Default workflow: Claude leads implementation; Codex is the independent reviewer + parallel runner — NOT the primary author of judgment-heavy fixes.** When Claude has full context on a precise, judgment-heavy change (PHY/DSP/protocol fix, channel-est, ARQ tuning), Claude implements it directly and verifies it on the multi-seed GUI itself — this is tighter and higher-quality than the "brief → babysit → watch for drift → verify → redirect" Codex-authoring loop (which also can't speed up the real-time GUI gate, and has shipped non-working changes Claude had to catch — e.g. an ineffective thrash-damp that still flipped at 3 s). Use Codex for what it is genuinely best at: (1) **independent code review** of Claude's diff (`codex review --uncommitted`, under the four-tier stack) — this satisfies the mandated control-plane/PHY counter-check via *review*, not authorship; (2) **parallel, well-scoped, long-running work** (large refactors, multi-cell sweeps) where a second worker actually parallelizes. Reserve Codex *authoring* for work where Claude lacks context or genuine parallelism helps — not for surgical fixes Claude understands and must re-verify anyway. Either way, Claude always independently verifies on the multi-seed GUI before anything counts.
